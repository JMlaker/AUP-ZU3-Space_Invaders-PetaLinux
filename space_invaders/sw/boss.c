...
void boss_render_explosion(const game_t *g, lfb_t *lfb) {
    ...
    if (g->boss_type == BOSS_TYPE_BLUE) {
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 4, BLUE, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 1, CYAN, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        if ((age & 1) == 0) {
            draw_filled_circle_clipped_y(lfb, cx, cy, (base_r > 3) ? (base_r - 3) : 1, CYAN,
                                         GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        }
    } else if (g->boss_type == BOSS_TYPE_HERMIT) {
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 4, YELLOW, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 1, PURPLE, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        if ((age & 1) == 0) {
            draw_filled_circle_clipped_y(lfb, cx, cy, (base_r > 3) ? (base_r - 3) : 1, PURPLE,
                                         GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        }
    } else if (g->boss_type == BOSS_TYPE_TOWER) {
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 4, YELLOW, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        draw_filled_circle_clipped_y(lfb, cx, cy, base_r + 1, ORANGE, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        if ((age & 1) == 0) {
            draw_filled_circle_clipped_y(lfb, cx, cy, (base_r > 3) ? (base_r - 3) : 1, YELLOW,
                                         GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
        }
